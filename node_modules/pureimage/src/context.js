...
class Context {
    ...
    inTriangle (xt,yt,x1,y1,x2,y2,x3,y3) {
        let b1 = this.sign(xt,yt, x1,y1, x2,y2) < 0;
        let b2 = this.sign(xt,yt,x2, y2, x3,y3) < 0;
        let b3 = this.sign(xt,yt,x3, y3, x1,y1) < 0;
        return ((b1 == b2) && (b2 == b3));
    }
    fillTriangle(x,y,w,h) {
        for(var i=x; i<x+w; i++) {
            for(var j=y+h; j>= y; j--) {
                if (this.inTriangle(i,j,x,y,x,(x+h),(x+w),y))
                this.fillPixel(i,j);
            }
        }
    }
